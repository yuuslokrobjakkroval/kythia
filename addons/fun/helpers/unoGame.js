...
class UnoGame {
	constructor(players, client) {
		...
		this.aiPlayerIds = new Set(
			players.filter((p) => p.bot || p.id === client.user.id).map((p) => p.id),
		);
	...
	runAiTurn() {
		...
		const playableCards = hand
			...
			.filter((item) => this.isCardPlayable(item.card));
		...
		if (playableCards.length > 0) {
			...
			if (WILD_TYPES.includes(bestPlay.card.type)) {
				const colorCounts = this.countHandColors(
					hand.filter((c) => !WILD_TYPES.includes(c.type)),
				);
				chosenColor =
	...
	playCard(playerId, cardIndex, chosenColor = null) {
		...
		if (!this.isCardPlayable(card)) {
			return {
				success: false,
				message:
					"You can't play that card! It doesn't match the color or value.",
				card: null,
			};
		}
		...
		return {
			success: true,
			message: `Played ${card.color} ${card.value}!`,
			card,
		};
	}
