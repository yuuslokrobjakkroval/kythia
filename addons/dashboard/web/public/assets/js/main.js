	...
	SearchConfig =
		...
		(() => {
			...
			if (
				...
				document.querySelectorAll("#layout-menu").forEach((e) => {
					(menu = new Menu(e, {
			...
			window.Helpers.swipeIn(".drag-target", (_e) => {
				window.Helpers.setCollapsed(!1);
			}),
				window.Helpers.swipeOut("#layout-menu", (_e) => {
					window.Helpers.isSmallScreen() && window.Helpers.setCollapsed(!0);
				});
			const a = document.getElementsByClassName("menu-inner"),
			...
			if (i.length) {
				...
				for (let e = 0; e < l.length; e++)
					l[e].addEventListener("click", function () {
						const a = this.getAttribute("data-language"),
			...
			const c = document.querySelectorAll(".dropdown-notifications-read"),
				m =
					...
					[].slice
						...
						.map((e) => new bootstrap.Tooltip(e)),
					[].slice.call(document.querySelectorAll(".accordion")).map((e) => {
...
function initializeAutocomplete() {
	if (document.getElementById("autocomplete"))
		return autocomplete({
			...
			render(e, t) {
				const { render: n, html: a, children: o, state: s } = e;
				s.query
					? e.sections.length
