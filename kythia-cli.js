	...
	pm2: {
		...
		commands: {
			startup: {
				...
				run: () =>
					runShell(
						"pm2 start index.js --name kythia-bot -- --deploy && pm2 save",
					),
			},
			start: {
	...
	build: {
		...
		commands: {
			build: {
				...
				run: () =>
					runShell(
						"node scripts/upversion.js && node scripts/docs.js && node scripts/obfuscate.js",
					),
			},
			obfuscate: {
				desc: "Obfuscate code",
...
Object.entries(groupedScripts).forEach(([category, { commands }]) => {
...
y.command(
	...
	(argv) => {
		...
		if (!fs.existsSync(addonPath)) {
			console.error(
				`❌ Addon '${addon}' not found or does not have a /commands directory.`,
			);
			process.exit(1);
		}
		...
		const targetDir =
			addon === "core" ? path.join(addonPath, "utils") : addonPath;
		...
		if (fs.existsSync(filePath)) {
			console.error(
				`❌ Command file '${commandName}.js' already exists in '${addon}'.`,
			);
			process.exit(1);
		}
...
`;
		...
		console.log(
			`✅ Successfully created command: ${path.relative(CLI_DIR, filePath)}`,
		);
	},
);
...
y
	.demandCommand(
		1,
		"You need to specify a command to run. Use --help to see available commands.",
	)
	.strict()
	.help()
	.alias("help", "h").argv;
